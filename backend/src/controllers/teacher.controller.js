...
const createTeacher = async (req, res) => {
  try {
    ...
    if (employeeIdExists) {
      return sendErrorResponse(res, new AppError(`Teacher with employee ID '${employeeId}' already exists in this school.`, 409));
    }
...
const getTeacherById = async (req, res) => {
  try {
    ...
    if (!teacher) {
      return sendErrorResponse(res, new AppError('Teacher not found', 404));
    }

    // Verify school isolation
    if (teacher.school.toString() !== req.user.school.toString()) {
      return sendErrorResponse(res, new AppError('Access denied: Teacher belongs to another school.', 403));
    }
